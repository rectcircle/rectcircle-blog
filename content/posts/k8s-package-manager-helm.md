...
* `.Values` 从 `values.yaml` 文件和 `--set-file` 、`--set` 设置的变量，参见下文：[values.yaml 详解](#valuesyaml-详解)。
* `.Chart` Chart.yaml文件内容，参见下文：[Chart.yaml 详解](#chartyaml-详解)。
...
values.yaml 文件中，存在几个特殊的 key， helm 会对齐做特殊处理：

* `exports` 作用参见上文： [Chart.yaml 详解](#chartyaml-详解)。
...
参考：[Chart.yaml 详解 - dependencies 的 condition 和 tags 说明](#chartyaml-详解)

### 等待依赖就绪

参考：[Chart 模板渲染结果 apply 顺序](#chart-模板渲染结果-apply-顺序)

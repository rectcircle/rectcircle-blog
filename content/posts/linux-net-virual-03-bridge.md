...
第二次无法 ping 通的原因，原因参见下文：[数据链路层流量分析](#数据链路层流量分析)。
...
经测试（Mac VirtualBox v 6.1.34），时通时不通，原因参见下文：[数据链路层流量分析](#数据链路层流量分析)。
...
ping br0 的 ip 地址仍然是，时通时不通，原因参见下文：[数据链路层流量分析](#数据链路层流量分析)。
...
通过执行如下命令，发现几个奇怪的点，但是并不确定是否是问题的原因。
...
假设我们希望 通过 veth0peer ping veth1peer，此时 Network Protocol Stack 会通过 veth0peer 发起一个 arp 请求。按照直觉来看，因为 arp 请求在数据链路层是广播，会到达 br0、gateway、veth0peer、veth1peer：只有 veth1peer 回复了 arp。

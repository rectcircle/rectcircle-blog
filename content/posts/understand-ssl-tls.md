...
假设一对 key pair 两个 key ，分别为 `key1` 和 `key2`，则：
...
在一次数据通讯过程，涉及两方；发送方和接收方，以及通信信道。由于通信信道是基于公开的经典物理学原理，所以第三方可以很容易的做到如下几点：
...
但是，以上场景存在一个严重问题，即：通讯双方如何知道通讯中使用的 key 呢？首先，不能明文传输，因为第三方可以窃听， key 泄漏了，加密形同虚设。因此在不适用非对称加密算法的情况下，最安全的方式是，通讯双方线下交换 key。
...
上文，安全通讯 - 非对称加密实现安全通讯流程，存在两个问题：

* 性能问题，非对称加密算法的性能很差，无法满足大规模数据传输的要求。
* 仍然无法解决 public key 分发的问题，如果 public key 通过明文进行分发，那么公钥数据也会被窃听。此时后续的通讯的安全性就无从谈起。
...
这样，非对称加密实现安全通讯的性能问题就解决了，而安全性由对称加密保证。
...
* 上文将介绍 ssl/tls 协议是如何解决如上问题的思路，真实的协议比上文描述的流程复杂的多。

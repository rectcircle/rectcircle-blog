...
* 新增 `files.dialog.defaultPath` 配置项，用于配置文件选择器默认路径。
...
* Python
    * 将 Python 调试能力移到 [ms-python.debugpy](https://marketplace.visualstudio.com/items?itemName=ms-python.debugpy) 扩展中。以实现使用最新版的 Python 插件的同时，支持 2.7 和 3.6 等旧版本 Python Debug 需求。
    * Pylance 支持本地化。

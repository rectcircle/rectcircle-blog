...
* 改进的选中后向上或者向下移动鼠标的滚动速度，离编辑器越远，滚动越快。
...
* Markdown
    * Markdown 链接验证，通过 `"markdown.validate.enabled": true` 配置项开启。有如下细粒度配置项：
...
* 新增一个扩展，可以在地址栏快速打开 github 仓库。

    <video autoplay="" loop="" muted="" playsinline="" controls="">
    <source src="/image/vscode/code-in-your-browser-omnibox.mp4" type="video/mp4">
    </video>

* vscode.dev 编辑的内容，可以通过 `Continue Working On` 命令在具有完整开发环境的 Codespace 中继续开发，未提交的代码将保存在 Editor Session 中，进入 Codespace 后将自动恢复这些未提交的代码，参见：[文档](https://code.visualstudio.com/docs/sourcecontrol/github#_continue-working-on)。
